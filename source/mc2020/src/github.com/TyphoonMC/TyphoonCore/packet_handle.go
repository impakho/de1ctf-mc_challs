...
func (packet *PacketStatusRequest) Handle(player *Player) {
	...
	*/
	...
	response := PacketStatusResponse{
		Response: fmt.Sprintf(`{"version":{"name":"MC2020","protocol":%d},"players":{"max":%d,"online":%d,"sample":[]},"description":{"text":"%s"},"favicon":"%s","modinfo":{"type":"FML","modList":[]}}`, 997, max_players, count, JsonEscape(motd), JsonEscape(favicon)),
	}
