...
ma_device_config audio_deviceConfig;
ma_device audio_device;
char * custom_username;
static Model model;
...
int audio_playback(char* filepath[]) {
    ma_device_uninit(&audio_device);
    ma_decoder_uninit(&audio_decoder);

    audio_result = ma_decoder_init_file(filepath, NULL, &audio_decoder);
    if (audio_result != MA_SUCCESS) {
        return -2;
    }

    audio_deviceConfig = ma_device_config_init(ma_device_type_playback);
    audio_deviceConfig.playback.format   = audio_decoder.outputFormat;
    audio_deviceConfig.playback.channels = audio_decoder.outputChannels;
    audio_deviceConfig.sampleRate        = audio_decoder.outputSampleRate;
    audio_deviceConfig.dataCallback      = audio_data_callback;
    audio_deviceConfig.pUserData         = &audio_decoder;

    if (ma_device_init(NULL, &audio_deviceConfig, &audio_device) != MA_SUCCESS) {
...
int play_c418() {
    int duration[12] = {192, 255, 41, 205, 178, 282, 255, 161, 66, 209, 216, 91};
    char filepath[20];
    int length;
    int num;
    num = rand() % 12;
    length = sprintf(filepath, "soundtrack/%d.mp3", num + 1);
    filepath[length] = '\0';
    audio_playback(filepath);
    return duration[num];
}
...
int main(int argc, char **argv) {
    ...
    // Init Audio Playback
    long int audio_time;
    audio_time = time(NULL) + play_c418();
    ...
    while (running) {
        ...
        if (g->mode == MODE_ONLINE) {
            ...
            client_version(1);
            login();
            char custom_talk[130];
            int length;
            length = snprintf(custom_talk, 128, "/nick %s", custom_username);
            custom_talk[length] = '\0';
            client_talk(custom_talk);
        }
