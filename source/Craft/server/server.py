...
class Model(object):
    ...
    def on_talk(self, client, *args):
        text = ','.join(args)
        if len(text) > 1024:
            client.send(TALK, 'Message\'s length too long')
            return
        text = filter(lambda x: x in string.printable[:-2], text)
        open('log/talklog.txt', 'a').write(client.nick + ' ' + text + '\n')
        if text.startswith('/'):
            ...
            if 'de1ctf' in text.lower():
                client.send(TALK, 'Ban word: "de1ctf"')
                return
    ...
    def on_nick(self, client, nick=None):
        if AUTH_REQUIRED:
            ...
            return
        if nick is None or len(nick) <= 0 or nick[0] == "@":
